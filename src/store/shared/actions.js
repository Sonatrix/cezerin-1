...
export const updateCartItemQuantiry = (item_id, quantity) => (
  dispatch,
  getState
) => {
...
export const fetchPaymentMethods = () => (dispatch, getState) => {
  ...
  return api.ajax.paymentMethods
    .list()
    .then(({status, json}) => {
...
export const fetchShippingMethods = () => (dispatch, getState) => {
  ...
  return api.ajax.shippingMethods
    .list()
    .then(({status, json}) => {
...
export const checkout = (cart, history) => (dispatch, getState) => {
  ...
  if (cart) {
    ...
    return api.ajax.cart
      .updateShippingAddress(cart.shipping_address)
...
export const updateCartShippingMethod = method_id => (dispatch, getState) => {
  api.ajax.cart
    .update({payment_method_id: null, shipping_method_id: method_id})
    .then(({status, json}) => {
      dispatch(receiveCart(json));
      dispatch(fetchPaymentMethods());
    })
...
export const updateCartPaymentMethod = method_id => (dispatch, getState) => {
  api.ajax.cart
    .update({payment_method_id: method_id})
    .then(({status, json}) => {
      dispatch(receiveCart(json));
    })
    .catch(error => {});
...
export const setCurrentPage = location => (dispatch, getState) => {
  ...
  if (currentPageAlreadyInState) {
    ...
    if (category) {
      ...
      api.ajax.sitemap
        .retrieve({path: locationPathname})
